...
2. 更新字段默认全部不勾选；单条提交、批量提交、SQL 和 CSV 都只使用勾选后的补丁。
   不必逐行展开编辑器：差异列里有「全选 N 项」（再点一次「取消勾选」），编辑器表头也有同一开关。
3. 整批处理按筛选范围一次勾完：工具栏「包含并勾选当前筛选的更新字段（N 行 · M 个字段）」。
   它只作用于**当前筛选结果**（当前阶段＋搜索条件），并会把命中行一并加入批量范围——字段勾了而记录没被包含时提交会静默跳过；
   第一次点击只进入确认态，第二次点击才真正勾选；「清空本阶段字段勾选（M）」一键回到默认不勾选。
   两者都只写 `reviewState.changed` 里的字段（新增行与待处理行没有可勾字段），草稿改回库内值后留下的陈旧勾选会被顺手清掉。
4. 缺少基础卡时，先点击创建基础卡，再返回印刷版本单独提交。缺少系列时先提交系列阶段。
5. 批量只处理当前阶段已包含且可提交的记录；筛选不隐式改变批量包含范围，但「包含并勾选」按当前筛选结果勾选字段。
6. 提交失败保留编辑和字段选择。批量遇到失败停止，已成功的记录保留，重试不会重交成功记录。
7. 重读数据库保留编辑值，但清空更新字段勾选，要求重新审核最新值。
8. SQL 按阶段导出，并按发布勾选状态触碰该阶段的版本标记；新基础卡尚未落库时，不导出依赖它的印刷版本。基础卡 SQL 执行后重读，再导出印刷阶段。
9. CSV 分为新增文件和 `update-patches` 文件；后者按 `table,id,attribute,value_json` 保存补丁，不能当整行 CSV 导入。
10. 站点快照仍是独立的全库原始数据导出，保留站点需要的全部字段。官网面板与小程序面板共用同一套字段白名单与导出校验。
...
- 批量勾选（`fieldSelection.ts`）只遍历筛选结果、只调 O(1) 的 `reviewState`，按钮上的「N 行 · M 个字段」是 computed，不随每次勾选重算；1500 行下一次「全选字段」实测约 150 ms。
...
浏览器检查使用模拟外部 API，不写入真实数据库。覆盖逐字段提交、歧义关联、分步新增、超过 200 条的分页、导出和失败后保留编辑；另含一键勾选路径：行内「全选 N 项」、批量「包含并勾选」的二次确认、清空字段勾选，以及官网繁中面板「繁中名／繁中副标题／繁中效果」三列一键勾选后按补丁提交。

注：官网面板的拉取按钮与小程序面板同名「开始拉取」，浏览器检查改为按 `data-testid` 定位；小程序面板需要先勾选「卡牌与关键词图标」才会启用拉取，这一步在官网面板加入后就已失效，本次一并修好。
