...
/** 批量勾选只作用于当前筛选结果，不隐式扩大到整个阶段。 */
const bulkPlan = computed(() => fieldSelectionPlan(filtered.value, state));
...
const rowFields = (r: ReviewRow) => selectedFieldCount(r, state(r));
const rowAllFields = (r: ReviewRow) => allFieldsSelected(r, state(r));
/** 行内一键：差异字段全勾 / 取消，省去为每行展开编辑器。 */
function toggleRowFields(row: ReviewRow): void {
  if (rowAllFields(row)) { row.selected = []; return; }
  applyFieldSelection([row], state);
}
function selectFilteredFields(): void {
  const plan = applyFieldSelection(filtered.value, state);
...
<template>
  <section class="mb-10" data-testid="sync-review">
    <div class="flex justify-between gap-4 items-start mb-4">
      <div>
        <h2 class="font-display text-xl font-bold">逐条审核同步差异</h2>
        <p class="text-xs text-ink-muted mt-2">更新字段默认不勾选，可在行内或按当前筛选一键勾选。缺少基础卡时，先创建基础卡，再单独提交印刷版本。</p>
      </div>
    ...
    <fieldset :disabled="busy || loading" class="min-w-0">
      ...
      <div class="flex gap-4 flex-wrap items-center text-xs mb-3">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-xs min-w-[680px]">
            <thead class="text-left text-ink-faint"><tr>
              <th class="p-2">更新
                <button v-if="activeState.before && activeState.changed.length" data-testid="editor-toggle-fields"
      ...
      <div class="card p-5 mt-5">
        ...
        <p class="text-xs text-ink-muted mt-2">批量仅处理本阶段已包含且可提交的 {{ readyCount }} 条；未勾选更新字段或等待关联的记录不会提交。列表筛选不改变批量包含范围，但「包含并勾选」按当前筛选结果勾选字段。</p>
